...
void online_menu_loop() {
    ...
    if (search_result != 0 && search_result != 7 && search_result != 6) 
    {
        char temp[54];
        snprintf(temp, sizeof(temp) - 1, "An unknown error has<p>occurred.<p><p>Error code: %d", search_result);
        ui_label_set_text(error_label, temp);
    };

    if (list && (search_result == 0))
    {   
        for (int i = 0; i < searchEntriesLength; i++)
        {
            ...
            if (card)
            {
                ...
                if (collaboration_icon && search_entries[i].originalId != 0)
                {
                ...
                if (high_object_icon && (search_entries[i].objCount >= 44000))
                {
                    ui_image_set_image(high_object_icon, 362, 0);
                    ui_element_set_position((UIElement *)high_object_icon, -list_width + 48 + get_text_length(&goldFont_fontCharset, 0.45f, false, tmp_creator) + 10 + ((search_entries[i].originalId != 0) ? 23 : 0), -4.5f);
                ...
                if (download_value)
                {
                    char *tmp_value = truncate_number(search_entries[i].downloads);
                ...
                if (like_value)
                {
                    ...
                    char *tmp_value = truncate_number(search_entries[i].likes);
                ...
                if (star_value && search_entries[i].stars > 0)
                {
                    char tmp_value[16];

                    snprintf(tmp_value, sizeof(tmp_value), "%d", search_entries[i].stars);
                ...
                if (featured_glow && search_entries[i].featureScore > 0)
                {
                ...
                if (difficulty_face)
                {
                    ui_image_set_image(difficulty_face, difficulty_stars[search_entries[i].stars], 0);
                    ui_element_set_position((UIElement *)difficulty_face, -list_width + 23,  (search_entries[i].stars > 0) ? -4 : 0);
                ...
                if (star_icon && search_entries[i].stars > 0)
                {
